...
def topology():
    return {"schema_version":"scene-topology.v3","topology_id":"TOPO_INN","location_id":"LOC_INN","nodes":[{"node_id":"NODE_DOOR","node_type":"ROOM","adjacent_nodes":[],"entrances":[],"exits":[],"floor":"GROUND","elevation":"0","orientation":"north","anchor_ids":["ANCHOR_LAMP"],"light_sources":["LAMP"],"camera_side_regions":["NORTH"]}],"movement_paths":[],"review_status":"REVIEWED"}

def coverage():
    return {"schema_version":"location-coverage-set.v3","coverage_id":"COVER_INN","location_id":"LOC_INN","views":[{"view_id":"VIEW_INN_A","node_id":"NODE_DOOR","camera_side":"NORTH","screen_direction":"left-to-right","visible_anchor_ids":["ANCHOR_LAMP"],"occluded_anchor_ids":[],"framing_result":"door and lamp readable","source_asset_id":"ASSET_INN_A","qc_status":"PASS"}],"review_status":"REVIEWED"}
...
def test_character_reference_package_schema_is_present():
    ...
    assert (root / "schemas" / "character-reference-package.v3.json").is_file()
