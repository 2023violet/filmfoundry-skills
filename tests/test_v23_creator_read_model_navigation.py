...
def collect_snapshot():
    ...
    return catalog, filmfoundry.collect_creator_snapshot(SMOKE_PROJECT, catalog)


def test_terminology_registry_is_versioned_and_uses_english_fallback_for_unknown_language():
    registry = filmfoundry.CreatorTerminologyRegistry()
...
def test_navigation_is_derived_from_action_free_snapshot_and_read_model_composes_both():
    ...
    navigation = filmfoundry.derive_creator_navigation(snapshot)
    model = filmfoundry.build_creator_read_model(SMOKE_PROJECT, catalog)
...
def test_navigation_returns_one_primary_and_all_same_priority_parallel_actions_in_stable_order():
    ...
    provenance = filmfoundry.CreatorProvenance((ref,), "VALIDATED", "test.rule")
    blockers = (
        filmfoundry.CreatorBlocker(
...
def test_hard_blocker_severity_precedes_observed_state_rule_heuristic():
    ...
    navigation = filmfoundry.derive_creator_navigation(replace(snapshot, blockers=(blocker,)))
...
def test_continuity_edge_before_select_does_not_require_observed_state():
    ...
    navigation = filmfoundry.derive_creator_navigation(replace(snapshot, shots=shots))
...
def test_derived_action_rejects_empty_provenance_with_contract_error():
    _, snapshot = collect_snapshot()
    empty_provenance = filmfoundry.CreatorProvenance((), "VALIDATED", "test.rule")
    blocker = filmfoundry.CreatorBlocker(
    ...
    with pytest.raises(ValueError, match="CREATOR_NAVIGATION_PROVENANCE: derived action requires non-empty provenance"):
        filmfoundry.derive_creator_navigation(replace(snapshot, blockers=(blocker,)))
...
def test_action_support_boundaries_resolve_in_registry_in_both_languages():
    _, snapshot = collect_snapshot()
    navigation = filmfoundry.derive_creator_navigation(snapshot)
    registry = filmfoundry.TERMINOLOGY_REGISTRY
